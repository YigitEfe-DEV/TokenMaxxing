                ...
                if entry.file_type().map_or(false, |ft| ft.is_file()) {
                    ...
                    if let Ok(content) = fs::read_to_string(entry.path()) {
                        ...
                        if content.len() > 50_000 {
                            report.oversized_files.push(entry.path().to_string_lossy().into_owned());
                        }

                        // Check for duplicates
                        // A simple hash using length + first 100 chars (not cryptographically secure, but works as heuristic)
                        let prefix = if content.len() > 100 { &content[..100] } else { &content };
                        let hash_key = format!("{}:{}", content.len(), prefix);

                        if let Some(existing) = content_hashes.insert(hash_key, entry.path().to_string_lossy().into_owned()) {
                            report.suspected_duplicates.push(format!("{} == {}", existing, entry.path().display()));
                        }
                    }
                }
            }
